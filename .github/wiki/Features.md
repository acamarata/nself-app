...
| List templates | Planned | No implementation on any surface (verified 2026-08-24) |
| Location-based lists | Planned | The columns exist; nothing reads or writes them, and no surface has UI (verified 2026-08-24) |
| Arrival reminders | Planned | Needs geofencing; no geofencing code exists in any client (verified 2026-08-24) |

## Tasks

| Feature | Status | Description |
|---|---|---|
| Due dates | Active | Natural-language input ("tomorrow 5pm", "next monday") on web and mobile |
...
| Recurring tasks | Active | Daily, weekly, monthly, yearly with an interval, on web and mobile; instances generated by a 03:00 UTC cron |
| Snooze / reminders | Active | Web sets an explicit reminder time; mobile offers 30 minutes, 1 hour, 1 day or 3 days before. Dispatched every 5 minutes. |
| Geolocation | Planned | The columns exist; no UI and no geofencing (verified 2026-08-24) |
| Evening digest | Active | One in-app notification per user at 20:00 UTC listing tomorrow's incomplete tasks |
...
| Feature | Status | Description |
|---|---|---|
| Mobile push (iOS/Android) | Blocked on credentials | Token registration against `np_device_tokens` and both transports (FCM HTTP v1, APNs HTTP/2) are implemented; delivery needs FCM/APNS credentials this project does not hold |
| Notification types | Partial | Produced today: task assigned, due-date reminder, evening digest. `shared_list` and `list_update` exist in the schema and nothing inserts them |
| Multi-channel delivery | In-app | Push is credential-gated (above); notification email has no producer yet. List invites do send email where SMTP is configured. |
| Notification center | Active | Bell with unread badge on web and mobile, over the same `np_notifications` rows and mark-read mutation |
| Smart batching | Planned | No batching code exists on any surface (verified 2026-08-24) |
| Web push | Planned | Not implemented. Web shows notifications in the in-app notification centre. |
